...
## config.hint
当前表单项提示信息，默认值为空，类型：`String | HTML String`
...
## config.disabled
当前表单项是否禁用，默认值为false，类型：`Boolean`
...
## config.span
当前表单项占用的列数，默认值为24，类型：`Number`

formkit会根据`config.span`值自动调整模块的宽度，例如：`config.span`为12时，模块宽度为`12/24`。

若设置了`columns`，则`config.span`值默认为`columns`的整数倍。例如：`columns`为2时则`config.span`值为12，模块宽度为`12/24`。   

::: warning 当前表单项占用的列数不能大于24，否则会失效。
:::
...
<formkit
    :config="[
        ...
        },
        {
            ...
            key: 'name4',
            span: 24,
            props: { placeholder: '请输入姓名4', clearable: true }
        }
    ]"
    v-model="dataset">
</formkit>
